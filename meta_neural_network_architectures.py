...
class LSTMArbiter(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, output_size, args, device):
        super(LSTMArbiter, self).__init__()

        # input_size : 입력 데이터의 feature 수
        # hidden_size: LSTM hidden state의 크기
        # num_layers : LSTM의 layer 수
    ...
    def forward(self, task_state):
        ...
        out, _ = self.lstm(task_state, (h0, c0))

        # LSTM의 출력 out shape: (batch_size, seq_length, hidden_size)

        # 마지막 시퀀스의 hidden state를 사용하여 fully connected layer 적용
        out = self.fc(out[:, -1, :])  # 마지막 시퀀스의 hidden state만 사용

        # Softplus 함수를 적용하여 출력
